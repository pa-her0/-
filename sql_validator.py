...
# from rich.markdown import Markdown # Not used in the provided sql_validator.py
...
class SQLValidator:
    ...
    def _check_basic_syntax(self, sql_query):
        ...
        try:
            ...
            if 'SELECT' in keywords and 'FROM' not in keywords: #
                # This check might be too simple for complex queries (e.g. SELECT without FROM for constant values)
                # but for typical table queries, it's a basic sanity check.
                # SQLite allows `SELECT 1;` which has no FROM.
                # Let's check if it's a DML SELECT
                ...
                if is_dml_select and 'FROM' not in keywords :
                     # A more robust check would be to see if identifiers are present after SELECT
                     # that are not functions or literals, implying columns are selected.
                     # For now, if it's a DML select and has no FROM, flag it if it's not selecting constants.
                     # This part is tricky with just keywords. `sqlparse` can give more structure.
    ...
    def _extract_tables(self, parsed):
        ...
        # This is a simplified extractor. For complex queries with subqueries, CTEs, joins,
        # a more robust extraction logic using sqlparse's tree traversal would be needed.
        tables = []
        from_seen = False
        ...
        # A more robust way using sqlparse iteration:
        # tables = set()
        ...
        # # This needs context (e.g. is it after FROM/JOIN?)
        # A proper walker like in https://github.com/andialbrecht/sqlparse/blob/master/examples/extract_table_names.py
        # would be better but is more complex for this snippet.
        # The original _extract_tables was simpler and might be sufficient for basic cases.
        # Let's revert to a slightly improved version of the original logic:
        ...
        for token in parsed.tokens:
            ...
            if select_seen:
                if isinstance(token, sqlparse.sql.Identifier):
                    ...
                    if token.tokens[-1].match(sqlparse.tokens.Punctuation, '()'): # Heuristic for function
                        # Could be like "func(column)" or "func(*)"
                        # For "func(column)", extract 'column' from within parenthesis
                        # For "func(*)", it might refer to all columns in a context
                        # This is complex, for now, we treat the whole identifier as one "column-like" expression
                        # or try to find identifiers within its parameters if it's a function.
                        # For simplicity, let's assume the Identifier itself is what we're checking
                        # or if it has children, like table.column
                        if '.' in token.value: #
                    ...
                    for identifier_item in token.get_identifiers(): #
                        ...
                        # Could also be a function or other complex item in the list
